...
import {
  ...
  Loader2, Check, Ban, Filter
} from 'lucide-react'
import {
  getAllExtraTimeRequests, approveExtraTimeRequest, rejectExtraTimeRequest
...
export default function OvertimeManagement() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true)
      const res = await getAllExtraTimeRequests()
      if (res?.requests) {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      if (reviewStatus === 'approved') {
        res = await approveExtraTimeRequest(selectedRequest._id, adminRemarks.trim())
      } else {
        res = await rejectExtraTimeRequest(selectedRequest._id, adminRemarks.trim())
      }

      showSuccess(res.message || `Overtime request ${reviewStatus} successfully!`)
      setReviewModalOpen(false)
      setSelectedRequest(null)
      fetchRequests()
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to process overtime request'
      showError(msg)
    } finally {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 3, margin: 0 }}>
              Review, approve, and track staff extra working hours on project deliverables.
            </p>
          </div>
        </div>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#64748b', textAlign: 'left' }}>
                  ...
                  <th style={{ padding: '12px 18px', fontWeight: 600 }}>Hours</th>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: '#0f172a', margin: 0 }}>
                {reviewStatus === 'approved' ? 'Approve Overtime' : 'Reject Overtime'}
              </h3>
              <button
            ...
            <p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.5, margin: '0 0 16px 0' }}>
              Confirm your decision for <strong>+{selectedRequest?.requestedHours} hours</strong> requested by {selectedRequest?.employeeId?.userId?.name}.
            </p>
            ...
            <form onSubmit={handleReviewSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="e.g. Overtime approved for upcoming release"
                  value={adminRemarks}

...
import {
  Home, Search, CheckCircle2, AlertCircle, X,
  Loader2, Check, Ban, Filter
} from 'lucide-react'
import {
  getAllWFHRequests, updateWFHStatus
...
export default function WFHManagement() {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true)
      const res = await getAllWFHRequests()
      if (res?.requests) {
        setRequests(res.requests)
      }
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to load WFH applications'
      showError(msg)
    } finally {
      setLoading(false)
    }
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateWFHStatus(selectedRequest._id, {
        status: reviewStatus,
        adminRemarks: adminRemarks.trim(),
      })

      showSuccess(res.message || `WFH application ${reviewStatus} successfully!`)
      setReviewModalOpen(false)
      setSelectedRequest(null)
      fetchRequests()
    } catch (err) {
      const msg = err.response?.data?.message || 'Failed to process WFH application'
      showError(msg)
    } finally {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 3, margin: 0 }}>
              Review, approve, and coordinate staff remote work arrangements.
            </p>
          </div>
        </div>
      </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          {loading ? (
            <div style={{ padding: 48, textAlign: 'center', color: '#64748b' }}>
              ...
              <p style={{ fontSize: 13 }}>Loading WFH applications...</p>
            </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: '#0f172a', margin: 0 }}>
                {reviewStatus === 'approved' ? 'Approve WFH' : 'Reject WFH'}
              </h3>
              <button
            ...
            <p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.5, margin: '0 0 16px 0' }}>
              Confirm your decision for WFH request submitted by <strong>{selectedRequest?.employeeId?.userId?.name}</strong> for <strong>{selectedRequest?.daysCount} day(s)</strong>.
            </p>
            ...
            <form onSubmit={handleReviewSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="e.g. Approved. Ensure availability on team Slack/meet."
                  value={adminRemarks}

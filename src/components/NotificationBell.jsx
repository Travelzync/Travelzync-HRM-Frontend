...
import { Bell, CheckCheck, ExternalLink, Sparkles, X, AlertCircle } from 'lucide-react'
import {
  getMyNotifications,
  getUnreadCount,
...
export default function NotificationBell({ fullPagePath = '/employee/notifications' }) {
  ...
  // Socket live notification subscription
  useEffect(() => {
    loadCount()
    initSocket()
    ...
    const unsubNotif = subscribeToSocket('new_notification', (notif) => {
      ...
      // 3. Show live floating notification toast
      setLiveToast(notif)
      setTimeout(() => {
        setLiveToast((cur) => (cur?._id === notif._id ? null : cur))
      }, 5000)
    })
      ...
      {liveToast && (
        <div
          ...
          style={{
            ...
            maxWidth: 360,
            cursor: 'pointer',
            borderLeft: `4px solid ${getSeverityBadgeColor(liveToast.severity)}`,
